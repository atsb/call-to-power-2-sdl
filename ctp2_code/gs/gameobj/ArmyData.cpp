			...
			m_array[i].CanPerformSpecialAction())
		{
			...
			&& g_network.IsLocalPlayer(m_owner))
				...
				for (sint32 j = 0; j < cell->GetNumTradeRoutes(); j++)
				{
					...
					// set pirating sound if route_owner is human player
					if (g_selected_item->GetVisiblePlayer() == route_owner)
					{ 
					    soundIdTmp= soundId;
					}
				}
			...
			else
			{
				...
				// play pirating sound if route_owner is human player even if pirating pos is not visible (i.e. sound corresponds to the feedback of the pirated trader)
				} else if (soundIdTmp != -1) { 
...
void ArmyData::ActionSuccessful(SPECATTACK attack, Unit &unit, Unit const & c)
{
	...
	else
	{
		if(soundID != -1)
		{
			...
			if(visiblePlayer == m_owner || unit.GetVisibility() & (1 << visiblePlayer))
			{
				if(g_selected_item->IsAutoCenterOn() 
				...
				// pos not used in SoundManager::AddSound, centering map could be implemented there, not sure though if that would cause troule for sounds not bound to a map position (e.g. click-sound)
