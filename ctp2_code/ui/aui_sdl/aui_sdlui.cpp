...
AUI_ERRCODE aui_SDLUI::CreateNativeScreen( BOOL useExclusiveMode )
{
	...
	configAizen.parseFile("userprofile.txt");

	// Load variable from userprofile.txt file
	bool booleanVal{ configAizen.getBoolean("WindowedMode")};
	...
	if (booleanVal) {
		m_SDLWindow = SDL_CreateWindow("Call To Power 2", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
			m_width, m_height, SDL_WINDOW_ALLOW_HIGHDPI);
	}
	else {
		m_SDLWindow = SDL_CreateWindow("Call To Power 2", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
			m_width, m_height, SDL_WINDOW_FULLSCREEN);
	}
	if (!m_SDLWindow) {
