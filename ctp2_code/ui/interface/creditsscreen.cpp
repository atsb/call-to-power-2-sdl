...
{
	sint32 deltaTime = GetTickCount64() - lastIdle;
	...
	lastIdle = GetTickCount64() + (m_animationSpeed - deltaTime);
...
AUI_ERRCODE c3_TriggeredAnimation::Idle()
{
	...
	lastIdle = GetTickCount64() + (m_blendSpeed - deltaTime);
...
AUI_ERRCODE CreditsWindow::Idle()
{
	...
	lastIdle = GetTickCount64() - deltaTime;
...
void CreditsWindow::InitCommonLdl(MBCHAR *ldlBlock)
{
	...
	if (m_creditsText == NULL)
	{
		...
		if (m_creditsText)
		{
			...
			m_creditsText->m_lastIdle = GetTickCount64();
		}
	}
...
{
	sint32 deltaTime = GetTickCount64() - m_lastIdle;
	...
	m_lastIdle = GetTickCount64() - deltaTime;
