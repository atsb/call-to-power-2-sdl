...
bool ui_CheckForScroll(void)
{
	...
	s_scrollcurtick	 = GetTickCount64();
...
//	const int       k_MAX_SMOOTH_SCROLL         = 64;
	...
	else
	{
		...
		if(scrolled) {
			...
			if(GetTickCount64() < scroll_start + g_theProfileDB->GetScrollDelay() * 100) {
	...
	if (scrolled)
	{
		...
		if (g_smoothScroll)
			...
			if ((s_scrollcurtick - s_lastscrolltick) > k_TICKS_PER_SCROLL) {
				s_lastscrolltick = GetTickCount64();
				g_tiledMap->ScrollMap(deltaX, deltaY);
			}
		}
	}
...
#define k_MAX_CMD_LINE 2048
	...
	g_SDL_flags = g_SDL_flags | SDL_WINDOW_FULLSCREEN_DESKTOP | SDL_WINDOW_ALLOW_HIGHDPI;
...
static void MessageBoxCTP2(void)
{
	// Set the message box text
	const char* lorem_ipsum_text = "This executable is not created by, nor endorsed by Activision. \nThis is a heavily modified fork of the Apolyton edition in SDL2 for modernisation and continued enjoyment.";

	// Set the message box title
	const char* title = "Call to Power II Activision EULA Agreement";

	// Show the message box
	SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION, title, lorem_ipsum_text, NULL);
};
...
int WINAPI CivMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int iCmdShow)
{
	...
	{
		uint32 frameStartTick = GetTickCount64();
		g_civApp->Process();
		...
		while (!g_letUIProcess) { // There are breaks, too ;)
			...
			const int frameTicksLeft = frameStartTick + TICKS_PER_FRAME - GetTickCount64();
			if (frameTicksLeft > 0) {
